...
switch lower(options.plottype)
    ...
    case 'polar'
        ...
        switch lower(sp.x_unit)
            case 'radian'
                % NOTE: 
                % I added this option to prevent overlaying 
                % the same text when reusing figure. 
                if isfield(options,'xticks')
                    if options.xticks
                        text(0, -1.1, 0, '-\pi/2');
                        text(1.1, 0, 0, '0');
                        text(0, 1.1, 0, '\pi/2');
        ...
        if sp.discrete
            ...
            hsp = plot3(cos(theta), sin(theta), sp.y,'b');
        end
        if isfield(sp, 'true_positions')
            theta = sp.true_positions / x_range(2) * pi/2;
            htp = stem3(cos(theta), sin(theta),...
                ones(size(sp.true_positions)) * y_range(2),...
                'r','LineWidth',2);
            hold on;
        end
